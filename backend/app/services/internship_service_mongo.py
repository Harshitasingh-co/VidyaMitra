"""
Internship Service - MongoDB Implementation
Core business logic for internship discovery using MongoDB
"""
...
class InternshipServiceMongo:
    """Core business logic for internship discovery using MongoDB"""
    
    def __init__(self):
        """Initialize the internship service"""
        self.db = get_database()
        self.profiles_collection = self.db.internship_profiles if self.db is not None else None
        logger.info("InternshipServiceMongo initialized")
    ...
    async def create_profile(self, user_id: str, profile_data: StudentProfileCreate) -> StudentProfile:
        ...
        try:
            if self.profiles_collection is None:
                raise DatabaseOperationError("Database not connected")
            ...
            if existing:
                ...
                profile_dict['created_at'] = existing.get('created_at', datetime.utcnow())
                await self.profiles_collection.update_one(
                    {'user_id': user_id},
                    {'$set': profile_dict}
                )
                result = await self.profiles_collection.find_one({'user_id': user_id})
            else:
                # Create new profile
                logger.info(f"Creating new profile for user: {user_id}")
                result = await self.profiles_collection.insert_one(profile_dict)
                result = await self.profiles_collection.find_one({'_id': result.inserted_id})
            
            if result is None:
                raise DatabaseOperationError("Failed to create/update profile - no data returned")
            
            # Convert MongoDB document to StudentProfile
    ...
    async def delete_profile(self, user_id: str) -> bool:
        ...
        try:
            ...
            await self.profiles_collection.delete_one({'user_id': user_id})
...
def get_internship_service_mongo():
    """Get InternshipServiceMongo instance"""
